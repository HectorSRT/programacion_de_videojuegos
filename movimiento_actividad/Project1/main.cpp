...
glm::vec3 posicion = glm::vec3(0.5f, -0.5f, 0.0f);
float rotacion = 0.0f;
float tamanio = 0.5f;
...
void updateWindow(GLFWwindow* window, Shader ourShader)
{
	while (!glfwWindowShouldClose(window))
	{
		...
		transform = translate(transform, posicion);
		transform = rotate(transform, rotacion, vec3(0.0f, 0.0f, 1.0f));
		transform = scale(transform, vec3(tamanio, tamanio, tamanio));
